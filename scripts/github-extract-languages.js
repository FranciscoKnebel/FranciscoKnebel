...
(async () => {
  ...
  do {
    ...
    const data = await fetch(
      `https://api.github.com/user/repos?per_page=100&affiliation=owner,collaborator&page=${page}`,
      fetchConfig,
    ).then(parseJSON);
  ...
  const cleanedRepos = repos.map((u) => ({
    language_url: u['languages_url'],
    full_name: u['full_name'],
  }));

  const list = await Promise.all(
    cleanedRepos.map((r) =>
      fetch(r['language_url'], fetchConfig)
        .then(parseJSON)
        .then((languages) => ({ repo: r['full_name'], languages })),
    ),
  );
  ...
  writeFileSync(
    __dirname + '/data/github-extract-languages.json',
    JSON.stringify(
      {
        top10: total
          .sort((a, b) => b[1] - a[1])
          .slice(0, 10)
          .map((e) => [e[0], e[1], `${((e[1] / lineCount) * 100).toPrecision(3)}%`]),
        total,
        languages,
      },
      null,
